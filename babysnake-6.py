...
def get_input(output): # returns processed input
    ...
    while True:
        if user_input:
            ...
            user_input = input("hmm... i don't think that was an answer.\n --> ")
...
def set_password(person): # sets password and changes user status ("logs in")
    ...
    while True:
        if password in list_of_passwords:
            ...
            print("\n*logging in as " + current_user.upper() + "*\n")
            return
...
class Person:
    ...
    def update_feeling(self):
        self.num_feel = get_input("how are you on a scale from 1 to 10?")
...
while True:
    ...
    if i_know_you(name_input):
        ...
        if right_person is None:
            print("huh. we must not have met after all.")
        ...
        person = Person(name_input)
        num_friends += 1
        print("great to meet ya! i'm baby snake.")
    ...
    print("here's your mood chart:")
    person.show_chart()
    print("see you next time!\n\n*logging out*")

    # TO DO: update get_input to answer some fun questions
    # TO DO: make a yes or no function?
    # TO DO: enable logout at any time